...
import { getProjectsFaqs, getProjectsPageCopy } from "../copy";

export function ProjectsFaqSection({ locale }: { locale: string }) {
    const copy = getProjectsPageCopy(locale);
    const faqs = getProjectsFaqs(locale);

    return (
        <ScrollRevealSection className="bg-gray-50 py-24">
            <div className="mx-auto max-w-3xl px-4 sm:px-6">
                <div className="reveal mb-16 text-center">
                    <h3 className="mb-4 text-3xl font-bold text-gray-900">{copy.faqTitle}</h3>
                    <p className="text-gray-500">{copy.faqIntro}</p>
                </div>
                <div className="reveal-stagger space-y-4">
                    {faqs.map((faq, index) => (
                        <div key={index} className="reveal rounded-lg border border-gray-100 bg-white p-6 shadow-sm">
